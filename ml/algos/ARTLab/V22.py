...
def test_deepfool_attack(test_loader, device, model, art_classifier):
    ...
    deepfool = DeepFool(art_classifier, max_iter=50, epsilon=1e-6, batch_size=10, verbose=False)
    ...
    for inputs, labels in test_loader:
        ...
        try:
            ...
            print(f"Error during DeepFool attack: {e}")
            continue
...
def visualize_attacks(model, test_loader, device, art_classifier, num_examples=5):
    ...
    for i, idx in enumerate(indices):
        ...
        axs[4].imshow(deepfool_img, cmap='gray')
        axs[4].set_title("DeepFool")
        axs[4].axis('off')

        axs[5].imshow(pgd_diff, cmap='hot')
        axs[5].set_title("PGD Difference")
        axs[5].axis('off')
...
def main():
    ...
    print("\n=== Evasion Attacks ===")
    # Test FGSM attack
    (fgsm_acc, fgsm_cm, fgsm_time), fgsm_eval_time = test_evasion_attack(
        FastGradientMethod(art_classifier, eps=EPSILON), f"FGSM (ε={EPSILON})", test_loader, device, model)

    # Test PGD attack
    (pgd_acc, pgd_cm, pgd_time), pgd_eval_time = test_evasion_attack(
        ProjectedGradientDescent(art_classifier, eps=EPSILON, max_iter=10), f"PGD (ε={EPSILON}, iter=10)", test_loader,
        device, model)
