...
OUTPUT_DIR = 'results/steg/2/'
os.makedirs(OUTPUT_DIR, exist_ok=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")

steg_dataset_path = 'noise_stegano'
...
def evaluate_model(model, test_loader, criterion):
    ...
    plt.ylabel('True')
    plt.savefig('%sconfusion_matrix.png' % OUTPUT_DIR)
    plt.close()
    ...
    plt.savefig('%sroc_curves.png' % OUTPUT_DIR)
    plt.close()
...
plt.savefig('%straining_validation_loss_accuracy.png' % OUTPUT_DIR)
