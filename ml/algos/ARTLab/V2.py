...
if not os.path.exists('adni_dataset2/AugmentedAlzheimerDataset/AD/AD-0001.jpg'):
    ...
    subdirectories = {
        'AD': 'adni_dataset2/AugmentedAlzheimerDataset/AD',
        'CN': 'adni_dataset2/AugmentedAlzheimerDataset/CN',
        'EMCI': 'adni_dataset2/AugmentedAlzheimerDataset/EMCI',
        'LMCI': 'adni_dataset2/AugmentedAlzheimerDataset/LMCI',
    }
    for prefix, path in subdirectories.items():
        rename_images_in_directory(path, prefix)

if not os.path.exists('adni_dataset2/train.csv'):
    def process_images_to_csv(directory_path, csv_writer):
        files = sorted(os.listdir(directory_path))

        for filename in files:
            name_without_extension = os.path.splitext(filename)[0]

            if name_without_extension.startswith('AD'):
                diagnosis = 3
            elif name_without_extension.startswith('LMCI'):
                diagnosis = 2
            elif name_without_extension.startswith('EMCI'):
                diagnosis = 1
            elif name_without_extension.startswith('CN'):
                diagnosis = 0
            else:
                diagnosis = -1
            csv_writer.writerow({'id_code': name_without_extension, 'diagnosis': diagnosis})
    subdirectories = [
        'adni_dataset2/AugmentedAlzheimerDataset/AD',
        'adni_dataset2/AugmentedAlzheimerDataset/CN',
        'adni_dataset2/AugmentedAlzheimerDataset/EMCI',
        'adni_dataset2/AugmentedAlzheimerDataset/LMCI',
    ]
    output_csv_path = 'adni_dataset2/train.csv'
    ...
    with open(output_csv_path, mode='w', newline='') as csv_file:
        ...
        for path in subdirectories:
            process_images_to_csv(path, writer)
    print(f"Les données ont été enregistrées dans {output_csv_path}")
...
shuffle_csv('adni_dataset2/train.csv', 'adni_dataset2/train.csv')
...
@timeit
def prepare_data():
    dataset = BrainMRIDataset(csv_file='adni_dataset2/train.csv',
                              root_dir='adni_dataset2/AugmentedAlzheimerDataset',
                              transform=transform)
...
class Net(nn.Module):
    def __init__(self):
        ...
        dummy_input = torch.zeros(1, 1, 200, 190)  # Taille d'entrée mise à jour
        ...
        self.fc1 = nn.Linear(flattened_size, 512)  # Utiliser la taille calculée
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)  # Aplatir la taille correctement
...
(fgsm_acc, fgsm_cm, fgsm_time), fgsm_eval_time = test_evasion_attack(fgsm, f"FGSM (ε={EPSILON})", test_loader, device)
(pgd_acc, pgd_cm, pgd_time), pgd_eval_time = test_evasion_attack(pgd, f"PGD (ε={EPSILON}, iter=10)", test_loader, device)
